#!/bin/bash
### Description: Radarr .NET Rocky install
...
### Version V1.0.3 2024-01-06 - StevieTV - exit script when it is ran from install directoray
### Version V1.0.4 2024-10-23 - Bobot - Edited the Script to work on Rocky8
### Version V1.0.5 2024-10-29 - Bobot - Edited the Script to install Radarr on Rocky8
...
app_uid=${app_uid:-$app}
# Prompt Group
read -r -p "What group should ${app^} run as? (Default: $app): " app_guid < /dev/tty
app_guid=$(echo "$app_guid" | tr -d ' ')
app_guid=${app_guid:-radarr}
...
if ! getent passwd "$app_uid" >/dev/null; then
    adduser --system --no-create-home --gid "$app_guid" "$app_uid"
...
yum update && yum install -y $app_prereq
echo ""
...
echo ""
echo "Downloading..."
wget --content-disposition "$dlbase"
tar -xvzf "${app^}".*.tar.gz
echo ""
